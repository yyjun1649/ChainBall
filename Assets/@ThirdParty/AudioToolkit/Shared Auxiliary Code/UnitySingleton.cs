        ...
        static public T GetSingleton( bool throwErrorIfNotFound, bool autoCreate, bool searchInObjectHierarchy = true )
        {
            if ( _instance == null )
            {
                ...
                if ( searchInObjectHierarchy )
                {
                    ...
                    {
                        if ( components[ i ] is ISingletonMonoBehaviour sb && sb.IsSingletonObject )
                        {
                ...
                if ( !component )
                {
                    ...
                    {
                        GameObject go = GameObject.Instantiate( _autoCreatePrefab );
                        go.name = _autoCreatePrefab.name;
