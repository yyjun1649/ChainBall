...
namespace CS.AudioToolkit
{
    ...
    public partial class AudioController_Editor : EditorEx
    {
        ...
        public static bool channelFoldout = false;
        ...
        GUIStyle boxStyle;
        GUIStyle _styleCategoryButtonHeader;

        int lastCategoryIndex = -1;
        int lastItemIndex = -1;
        int lastSubItemIndex = -1;

        public static bool[] channelSettingsFoldout = new bool[AudioChannel.AUDIO_CHANNEL_COUNT];
        ...
        public new void SetStyles()
        {
            ...
            //var foldoutBackgroundNormal = UIUtility.MakeTex( 32, 32, Color.white );
            //var foldoutBackgroundClicked = UIUtility.MakeTex( 32, 32, Color.white );
        ...
        bool HeaderFoldout( ref bool isFoldedOut, string text )
        {
            Color blue = new Color( 0.2f, 0.2f, 0.35f );
            Color lighterBlue = blue * 1.7f;

            var tmpCol = GUI.backgroundColor;
            GUI.backgroundColor = isFoldedOut ? lighterBlue : blue;
            //isFoldedOut = EditorGUILayout.Foldout( isFoldedOut, text, EditorStyles.foldoutHeader );
            if( GUILayout.Button( ( isFoldedOut ? "-\t" : "+\t" ) + text, foldoutStyle ) )
            {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if ( HeaderFoldout( ref globalFoldout, "Global Audio Settings") )
            {
            ...
            // channel specific
            if( HeaderFoldout( ref channelFoldout, "Channel Settings" ) )
            {
                if( AC.isAdditionalAudioController )
                {
                    GUILayout.Label( "Channels can only be specified in the main AudioController" );
                }
                else
                {
                    EditorGUI.indentLevel++;
                    if( channelSettingsFoldout == null || channelSettingsFoldout.Length != AudioChannel.AUDIO_CHANNEL_COUNT )
                    {
                        channelSettingsFoldout = new bool[AudioChannel.AUDIO_CHANNEL_COUNT];
                    }
                    for( int i = 1; i < AudioChannel.AUDIO_CHANNEL_COUNT; i++ )
                    {
                        channelSettingsFoldout[i] = EditorGUILayout.Foldout( channelSettingsFoldout[i], GetChannelName( i ), foldoutStyle2 );

                        if( channelSettingsFoldout[i] )
                        {
                            var channelSettings = AC.GetAudioChannelWithIndex( i ).settings;
                            float v_in = channelSettings.crossfadeTime_In;
                            EditFloat( ref v_in, "Crossfade-in Time", "sec" ); channelSettings.crossfadeTime_In = v_in;

                            float v_out = channelSettings.crossfadeTime_Out;
                            EditFloat( ref v_out, "Crossfade-out Time", "sec" ); channelSettings.crossfadeTime_Out = v_out;

                            EditBool( ref channelSettings.loopPlaylist, "Loop Playlist" );
                            EditBool( ref channelSettings.shufflePlaylist, "Shuffle Playlist", "Enables random playback of music playlists. Takes care that the same audio will not get played again too early" );
                            EditBool( ref channelSettings.crossfadePlaylist, "Crossfade Playlist" );
                            EditFloat( ref channelSettings.delayBetweenPlaylistTracks, "Delay Betw. Playlist Tracks", "sec" );
                        }
                    }
                    EditorGUI.indentLevel--;
                }
            ...
            // playlist specific
            if ( HeaderFoldout( ref playlistFoldout, "Playlist Settings" ) )
            {
                ...
                if ( GUILayout.Button( "+", GUILayout.Width( 25 ) ) && AC.playlists != null && AC.playlists.Length > 0 )
                {
                    LogUndo( "Add Playlist" );
                    ArrayHelper.AddArrayElement( ref AC.playlists, new Playlist { name = _nameForNewPlaylist } );
                    currentPlaylistIndex = AC.playlists.Length - 1;
                    KeepChanges();
                }

                if ( GUILayout.Button( "-", GUILayout.Width( 25 ) ) && AC.playlists != null && AC.playlists.Length > 0 )
                {
                    if ( AC.playlists.Length > 1 )
                    {
                        LogUndo( "Del Playlist" );
                        ArrayHelper.DeleteArrayElement( ref AC.playlists, currentPlaylistIndex );
                        currentPlaylistIndex = Mathf.Clamp( currentPlaylistIndex - 1, 0, AC.playlists.Length - 1 );
                        KeepChanges();
                    }
                    else
                    {
                        AC.playlists[ 0 ] = new Playlist();
                    }
                }
                ...
                EditString( ref AC.playlists[ currentPlaylistIndex ].name, "Playlist Name", AC.playlists[ currentPlaylistIndex ].name == _nameForNewPlaylist ? textAttentionStyle : null );
                ...
                if ( GUILayout.Button( "Up", GUILayout.Width( 35 ) ) && AC.playlists != null && AC.playlists.Length > 0 )
                {
                    LogUndo( "Move Playlist entry" );
                    if ( SwapArrayElements( AC.playlists[ currentPlaylistIndex ].playlistItems, currentPlaylistEntryIndex, currentPlaylistEntryIndex - 1 ) )
                    {
                        currentPlaylistEntryIndex--;
                        KeepChanges();
                    }
                }
                if ( GUILayout.Button( "Dwn", GUILayout.Width( 40 ) ) && AC.playlists != null && AC.playlists.Length > 0 )
                {
                    LogUndo( "Move Playlist entry" );
                    if ( SwapArrayElements( AC.playlists[ currentPlaylistIndex ].playlistItems, currentPlaylistEntryIndex, currentPlaylistEntryIndex + 1 ) )
                    {
                ...
                if ( GUILayout.Button( "-", GUILayout.Width( 25 ) ) && AC.playlists != null && AC.playlists.Length > 0 )
                {
                    ...
                    ArrayHelper.DeleteArrayElement( ref AC.playlists[ currentPlaylistIndex ].playlistItems, currentPlaylistEntryIndex );
                    currentPlaylistEntryIndex = Mathf.Clamp( currentPlaylistEntryIndex - 1, 0, AC.playlists.Length - 1 );
                    KeepChanges();
                }
                ...
                {
                    AudioController.PlayPlaylist( AC.playlists[ currentPlaylistIndex ].name );
                }
                EditorGUILayout.EndHorizontal();
                GUI.enabled = true;
            }
            ...
            if ( HeaderFoldout( ref categoryFoldout, "Category Settings") )
            {
                // Audio Items 
                EditorGUILayout.BeginHorizontal();
                ...
                if ( curCat != null )
                {
                    ...
                    if ( HeaderFoldout( ref itemFoldout, "Audio Item Settings" ) )
                    {
                        EditorGUILayout.BeginHorizontal();
                        ...
                        if ( curItem != null )
                        {
                            ...
                            if ( HeaderFoldout( ref subitemFoldout, "Audio Sub-Item Settings" ) )
                            {
        ...
        private string GetChannelName( int i )
        {
            ...
            case 1: return "Music";
            case 2: return "Ambience";
            case 3: return "Custom 1";
            case 4: return "Custom 2";
            case 5: return "Custom 3";
            case 6: return "Custom 4";
            }
            return "";
        }
        ...
        {
            if ( AC.playlists == null ) return;

            for ( int playlistI = 0; playlistI < AC.playlists.Length; ++playlistI )
            {
                for ( int i = 0; i < AC.playlists[ playlistI ].playlistItems.Length; i++ )
                {
                    if ( AC.playlists[ playlistI ].playlistItems[ i ] == originalName )
                    {
                        AC.playlists[ playlistI ].playlistItems[ i ] = newName;
                    }
                }
        ...
        {
            if ( AC.playlists == null )
            {
                return new string[ 0 ];
            }

            var names = new string[ AC.playlists.Length ];
            for ( int i = 0; i < AC.playlists.Length; ++i )
            {
                names[ i ] = AC.playlists[ i ].name;
        ...
        private string[ ] GetPlaylistEntryNames()
        {
            ...
            if ( AC.playlists[ currentPlaylistIndex ].playlistItems == null )
            {
                return new string[ 0 ];
            }

            var names = new string[ AC.playlists[ currentPlaylistIndex ].playlistItems.Length ];
            for ( int i = 0; i < AC.playlists[ currentPlaylistIndex ].playlistItems.Length; i++ )
            {
                names[ i ] = string.Format( "{0}: {1}", i, AC.playlists[ currentPlaylistIndex ].playlistItems[ i ] );
            }
            return names;
        }
